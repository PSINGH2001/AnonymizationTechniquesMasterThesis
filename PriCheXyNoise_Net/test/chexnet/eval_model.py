...
def make_pred_multilabel(data_transforms, model, image_path, save_path, perturbation_type='none', 
                         perturbation_checkpoint=None, mu=None, b=None, m=None, eps=None, deformation_degree=None):
    ...
    :param deformation_degree: float
    """
    ...
    if perturbation_type == 'noise':
        perturbation_model = UNet(1, 1, 32).cuda()
        perturbation_model.load_state_dict(perturbation_checkpoint)
        perturbation_model.eval()
    ...
    for i, data in enumerate(dataloader):
        ...
        if perturbation_type == 'noise':
            fakes_1 = perturbation_model(inputs)
            inputs = inputs + (fakes_1 * deformation_degree)
