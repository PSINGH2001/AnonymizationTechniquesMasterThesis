...
def scipy_to_torch_sparse(scp_matrix):
    ...
    :param scp_matrix: scipy.sparse.coo_matrix
        The input SciPy sparse matrix in COO format.
    :return: torch.sparse.FloatTensor
        The corresponding PyTorch sparse tensor.
    """
    # Extract values and indices from the SciPy sparse matrix 
...
def mOrgan(N):
    """
    Create an adjacency matrix for an organ with cyclic connections.
    
    :param N: int
        The size of the matrix (number of nodes).
    :return: np.ndarray
...
def mOrganD(N):
    """
    Create a downsampling matrix for an organ.
    
    :param N: int
        The original size of the matrix.
    :return: np.ndarray
        The generated downsampling matrix.
    """
    N2 = int(np.ceil(N/2))
    sub = np.zeros([N2, N])
...
def mOrganU(N):
    """
    Create an upsampling matrix for an organ.
    
    :param N: int
        The original size of the matrix.
    :return: np.ndarray
        The generated upsampling matrix.
    """
    N2 = int(np.ceil(N/2))
    sub = np.zeros([N, N2])
...
def genMatrixesLungsHeart():   
    """
    Generate adjacency and downsampling/upsampling matrices for lungs and heart.
    
    :return: tuple of np.ndarray
        Returns the adjacency matrix (A), downsampled adjacency matrix (AD),
        downsampling matrix (D), and upsampling matrix (U) for the lungs and heart.
    """    
    RLUNG = 44
    LLUNG = 50
    HEART = 26
...
def getMaskSegmentation(inputs1, segmentation_model): 

    """
    Apply segmentation to input images and generate masks.
    
    :param inputs1: torch.Tensor
        Input images in the form of a tensor.
    :param segmentation_model: model
        A segmentation model used to create masks for the input images.
    :return: torch.Tensor
        The segmented mask for each input image.
    """
     
    # Iterate over each input image
    # Initialize an empty tensor to store segmented images
    fakes_1_image_list = torch.empty_like(inputs1)

    for i in range(inputs1.shape[0]):
        inputs1_image = ToPILImage()(inputs1[i])

        # Perform image segmentation using the provided model
        mask = segmentation_model.segment(inputs1_image)

        # Convert noisy_image back to a tensor
        noisy_image_tensor = torch.from_numpy(mask)#.float()
        fakes_1_image_list[i] = noisy_image_tensor
    return fakes_1_image_list.cuda()
...
def train(generator, training_loader, gauss_filter, grid_identity, mu, ac_loss, verification_loss, ac_loss_weight, 
          ...
          show_every_n_epochs, show_every_n_iters, save_path, segmentation_model):
    ...
    :param segmentation_model: torch.nn.Module
        The moule that is used to generate the mask of the chest radiographs.
    :param training_loader: torch.utils.data.DataLoader
        The data loader that is used during training.
    :param gauss_filter: torch.nn.Module (GaussianSmoothing)
        The gaussian filter that is applied on the learned flow field to guarantee smooth image deformations.
    ...
    for i, batch in enumerate(training_loader):
        ...
        if grid_identity is not None:
            ...
            # Obtain segmentation mask for the input images
            mask = getMaskSegmentation(inputs1, segmentation_model)
            # Apply the mask to the grids, ensuring only relevant parts are transformed
            grids = (grids*mask)
        ...
        # Remove this when you want to freeze the classifier and uncomment this when you want to optimize the classifier.
        '''
...
def validate(generator, validation_loader, gauss_filter, grid_identity, mu, ac_loss, verification_loss, ac_loss_weight, 
             ver_loss_weight, epoch, n_epochs, show_every_n_epochs, show_every_n_iters, save_path, segmentation_model):
...
def train_snn(perturbation_type, net, perturbation_net, grid_identity, gauss_filter, mu, training_loader, criterion,
    ...
    :param perturbation_type: str
        Defines the type of perturbation. Options: 'flow_field', 'privacy_net', 'dp_pix', 'none', 'noise'
